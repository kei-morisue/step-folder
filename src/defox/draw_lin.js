...
export const DRAW_LIN = {
    ...
    draw_state: (svg, FOLD, L) => {
        ...
        for (const [i, top_fs] of L.entries()) {
            for (const [j, top_f] of top_fs.entries()) {
                const g = SVG.append("g", gg, { id: "face_" + top_f })
                const face = faces[top_f];
                SVG.draw_polygons(g, [face], {
                    id: true,
                    fill: Ff[top_f] ? DRAW.color.face.top : DRAW.color.face.bottom,
                    stroke: Ff[top_f] ? DRAW.color.face.top : DRAW.color.face.bottom,
                    stroke_width: 3
                });
